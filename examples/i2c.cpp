...
private:
  ...
  [[nodiscard]]
  auto i2c_read(uint8_t subaddress) const -> uint16_t
  {
    auto response = ::i2c_smbus_read_word_data(device_, subaddress);
    assert((response >= 0) and "Failed to read data");
    fmt::print("i2c 32 bit raw: {}\n", response);

    // NOLINTNEXTLINE(readability-magic-numbers)
    auto as_16 = uint16_t(response & 0xFFFF);
  ...
  auto init_accel() -> void
  {
    ...
    // CTRL_REG8
    tempRegValue = 0;
    tempRegValue |= (1 << 2);  // Increment address in multibyte reads
